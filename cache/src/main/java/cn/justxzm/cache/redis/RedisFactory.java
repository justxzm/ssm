package cn.justxzm.cache.redis;
...
import cn.justxzm.cache.util.FileUtil;
...
public class RedisFactory {
    ...
    static {
        ...
        boolean blockWhenExhausted = FileUtil.getBoolean(props, "redis.conf.blockWhenExhausted", true);//默认：true(连接耗尽是否阻塞等待)
        long maxWaitMillis = FileUtil.getLong(props, "redis.conf.maxWaitMillis", -1);//默认：-1，即无限等待(等待获取连接的最长时间)

        boolean testOnBorrow = FileUtil.getBoolean(props, "redis.conf.testOnBorrow", false);//默认：false(获取连接前，是否对连接进行测试)
        boolean testOnReturn = FileUtil.getBoolean(props, "redis.conf.testOnReturn", false);//默认：false(归还连接前，是否对连接进行测试)
        ...
        boolean testWhileIdle = FileUtil.getBoolean(props, "redis.conf.testWhileIdle", true);//默认：false(对空闲连接进行扫描，检查连接有效性)
        long timeBetweenEvictionRunsMillis = FileUtil.getLong(props, "redis.conf.timeBetweenEvictionRunsMillis", 30000);//默认：-1，(两次扫描空闲连接的时间间隔)
        int numTestsPerEvictionRun = FileUtil.getInt(props, "redis.conf.numTestsPerEvictionRun", 3);//默认：3(每次空闲扫描时扫描的控线连接的个数)
        long minEvictableIdleTimeMillis = FileUtil.getLong(props, "redis.conf.minEvictableIdleTimeMillis", 60000);//默认：30min(一个空闲连接至少连续保持30min中空闲才会被空闲扫描)
        /*
         * 配置redis参数
         */
